...
def __color_jitter__(img):
    ## assume CHW format
    # for i in range(img.shape[0]): # for each img in batch-size
            
    img_min, img_max = img.min(), img.max()
    aux= np.transpose(img,(1,2,0))   # change from CHW to HWC
    # p2, p98 = np.percentile(img[i], (2, 98))
    # img_rescale = exposure.rescale_intensity(img[i],in_range=(p2, p98))
    aux = exposure.equalize_hist(aux)
    aux = aux*(img_max-img_min)+img_min
    img= np.transpose(aux,(2,0,1)) #change from HWC to CHW
    return img  
...
#         print('p: ', self.p)
    ...
    def __call__(self, img, target ):
        ...
        if np.random.rand()<self.p:
            print('do 3')
            rotations = np.random.choice([1,2,3])
            
            img = np.rot90(img, k=rotations, axes=(1,2)).copy()
            target = np.rot90(target, k=rotations, axes=(1,2)).copy()
            
        if np.random.rand()<self.p:
            print('do 4 channel shuffle')
            ch = [0,1,2,3]
            random.shuffle(ch)
        ...
        if np.random.rand()<self.p:
            print('do 5, color jitter')
            img = __color_jitter__(img)
            target = __color_jitter__(target)
        
        assert img is not None
        assert target is not None
        
        return img, target 
        #torch.from_numpy(imgHR), torch.from_numpy(imgLR), torch.from_numpy(imgVLR), torch.from_numpy(target1), torch.from_numpy(target2)
